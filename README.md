...
The channel is built from a flat metal sheet of width \(W = 3\) m and bent into three parts: right wall \(a\), base \(b\), and left wall. When the right wall is bent at an angle \(\theta\), the geometry must satisfy

\[
b = W - a(1 + \sin \theta), \quad b \ge 0
\]

The goal is to choose \(a\) and \(\theta\) to maximize the cross-sectional area of the channel while remaining feasible.

## Visualizations

The project uses several visualizations to tell the story:

- **Contour and surface plots** of the area \(A(a,\theta)\) over the feasible region, showing how different choices of \(a\) and \(\theta\) change the area and where the global maximum lies.
...
- **Comparison table/plots** summarizing convergence speed and performance of several algorithms (Armijo gradient descent, fixed-step GD, nonlinear conjugate gradient, momentum).
...
- Turn a complex geometric optimization problem into intuitive contour/surface plots.
